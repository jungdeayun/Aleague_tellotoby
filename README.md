...
### 저희 팀의 전략은
링통과는 영상의 상하좌우의 픽셀 수를 비교해 초록색 링을 인식 후 구멍을 찾은 뒤, 드론 영상 크기의 중심점과 드론 영상의 링 구멍의 중심점의 차이를 계산하여 통과하도록 합니다.  
빨간색 점과 파란색 점은 위와 같이 픽셀 수를 이용해 90도 회전 또는 착지하도록 합니다. 만약 점을 지나치거나 못 찾았을 경우에는 후진하여 다시 찾도록 합니다.
...
- **이미지 전처리**
 ...
 6. [imfill](https://kr.mathworks.com/help/images/ref/imfill.html)을 사용하기 위해 첫 행과 마지막 행을 1로 바꿈
 7. imfill한 이미지와 하지 않은 이미지를 비교해 바뀐 부분을 1으로 바뀌지 않은 부분을 0으로 하여 링 구멍 인식

- **드론 제어**

 8. 링이 왼쪽에 있는지 오른쪽에 있는지 판단하기 위해 영상을 반으로 나누어 각각 픽셀 수를 더하여 왼쪽이 많으면 왼쪽으로 ROLL 제어 이동하고, 오른쪽이 많으면 오른쪽으로 ROLL 제어 이동함
그리고 위쪽이 많으면 THROTTLE 제어 이동하고, 아래쪽이 많으면 THROTTLE 제어로 이동하여 링 구멍을 찾음
